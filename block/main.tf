...
data "template_file" "cloudinit-mount" {
  template = file("${path.module}/../templates/cloudinit-mount.yml")

  vars = {
    device       = var.data_disk_device
    mount_point  = var.data_disk_mount
    mount_device = var.data_disk_device
    filesystem   = var.data_disk_fstype
  }
}
...
data "template_cloudinit_config" "cloudinit" {
  ...
  part {
    content      = data.template_file.cloudinit-mount.rendered
    content_type = "text/cloud-config"
  }
...
resource "proxmox_vm_qemu" "virtual_machine" {
  ...
  disk {
    ...
    storage      = var.data_datastore
    storage_type = var.data_storage_type
    iothread     = false
    backup       = false
  }
